...
case "$pm" in
  apt-get)
    ...
    apt-get install -y --no-install-recommends \
      ...
      docker.io docker-compose cockpit cockpit-machines \
  ...
  dnf)
    dnf install -y \
      ...
      docker-compose-plugin cockpit cockpit-machines \
      openssh-clients curl iproute iputils python3-pip || \
    dnf install -y libvirt virt-install qemu-kvm cockpit cockpit-machines moby-engine docker-compose openssh-clients curl python3-pip
  ...
  pacman)
    pacman -Sy --noconfirm --needed \
      ...
      docker docker-compose cockpit cockpit-machines \
